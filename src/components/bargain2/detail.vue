...
<div class="detail" v-if="data&&user&&config">
  <div class="detail__container">
    <div class="detail__header">
      <div class="detail__user">
        <img id="headimg" class="detail__headimg" :src="imgFilter(user.headimg)" alt="">
        <span class="detail__name">{{user.true_name}}</span>
      </div>
      <div>
        <router-link :to="{name: 'bargain2Home', query:{activity_id: config.id}}" class="detail__link">活动主页>></router-link>
        <router-link :to="{name: 'bargain2Rule', query:{activity_id: config.id}}" class="detail__link">活动规则>></router-link>
      </div>
    </div>
    <div class="book">
    ...
    <div class="detail__tips" v-else-if="data.get_goods_id>0 && data.get_goods_id == data.id">
      ...
      <p>未认证医生认证成功后即可领券拿书</p>
      <p>(领券后进入拿书页面，拍下自动抵扣所有费用)</p>
      <div class="book__handle">
        <template v-if="user.isrenzheng<=0">
          <a class="book__btn" :href="`/muke/#/identificate?from=${data.id}&activity_id=${config.id}`">认证</a>
          <a class="book__btn over" v-if="data.status == 1">领券</a>
          <a class="book__btn over" v-else>已领券</a>
          <a class="book__btn over">拿书</a>
        </template>
        <template v-else>
          <a class="book__btn over">已认证</a>
    ...
    <div class="detail__tips" v-else-if="data.get_goods_id>0 && data.get_goods_id != data.id">
      ...
      <p>默认最快完成的一本为砍到商品，<router-link :to="{name: 'bargain2Detail', params: {id: data.get_goods_id}, query:{activity_id: config.id}}" class="detail__link2">去查看我最快砍到的商品</router-link></p>
    </div>
    <div class="detail__tips" v-else-if="data.stock<=0">
      抱歉，这本书已被抢光，请尝试别的书籍，<router-link :to="{name: 'bargain2Home', query:{activity_id: config.id}}" class="detail__link2">点击去活动主页</router-link>
    </div>
    ...
    <div class="list">
      ...
      <div class="detail__subscribe" v-if="!isStart && canSub">
        <a @click="subscribe" v-if="!data.is_attention">订阅</a>
        <a class="on" @click="subscribe" v-else>取消订阅</a>
        <p>订阅此书，我们将在砍价开始前五分钟提醒您</p>
      </div>
      <ul>
  ...
  <div class="detail__handle" :style="{background:config.button_color}" v-if="!isStart" @click="share">
    将这个好消息告诉好友
  </div>
  <div class="detail__handle" :style="{background:config.button_color}" v-else-if="isSelf">
    <a class="detail__btn" @click="showPoster">生成砍价海报</a>
    <a class="detail__btn" @click="share">分享给好友帮忙砍价</a>
  </div>
  <div class="detail__handle" :style="{background:config.button_color}" v-else>
    <router-link class="detail__btn" :to="{name: 'bargain2Home', query:{activity_id: config.id}}">我也要参加</router-link>
    <a class="detail__btn" @click="bargain">帮TA砍一刀</a>
  </div>
  <div class="poster" v-show="isShowPoster">
    <img class="poster__title" src="~assets/img/bargain/poster_title.png" alt="">
    <!-- <canvas class="poster__img" id="canvas" data-width="250" data-height="444" v-show="false"></canvas>
  ...
  <div class="master-weixin" v-show="isShow" @click="hide">
    <div class="wexin-box">
      ...
      <template v-show="!isSelf">
        <img id="code" class="wexin-code" :src="`/api/frontend/user/iniqrcode?info=bargain_${data.id}_${user.id}&is_temp=true&size=4`" alt="">
        <p>扫码帮TA砍一刀</p>
      </template>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getUser(id) {
      ...
      this.$axios.get(url).then((res) => {
        if (res.data.status == 0) {
          ...
          const link = `${location.origin}/active_web/#/bargain2/detail/${this.data.id}?id=${this.user.id}&activity_id=${this.data.activity_id}`;
    ...
    recieve() {
      ...
      if (this.user.isrenzheng < 0) {
        this.$messagebox.alert('未认证医生认证成功后即可领券拿书');
      } else if (subscribe && this.user.isrenzheng > 0) {
    ...
    subscribe() {
      const url = `/api/frontend/bargain/attention/${this.data.id}`;
      if (subscribe) {
        this.$axios.get(url).then((res) => {
          this.$toast(res.data.msg);
          if (res.data.status == 0) {
    ...
    showPoster() {
      ...
      // imgTogether(bg, this.user.name, (url)=>{
      //   this.inviteImg = url;
      // });
...
function imgTogether(bg, name, cb) {
